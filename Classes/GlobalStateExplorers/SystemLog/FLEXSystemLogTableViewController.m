...
- (void)updateLogMessages
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *newMessages = [self newLogMessagesForCurrentProcess];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            for (FLEXSystemLogMessage *message in newMessages) {
                [self.logMessageIdentifiers addIndex:message.messageID];
            }
...
- (NSArray *)newLogMessagesForCurrentProcess
{
    ...
    while ((aslMessage = asl_next(response))) {
        NSUInteger messageID = atoll(asl_get(aslMessage, ASL_KEY_MSG_ID));
